...
1. **Command Line runner**: Use `bun` to run the backend. **DO NOT USE `bun run tsc`**. Run files natively with `bun run <file>`. (e.g. `bun run src/index.ts --params params.json`)
