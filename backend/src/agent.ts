...
export class IntelAgent {
    ...
    public async analyze(intel: Intel, gameMapArea: GameMapArea): Promise<string> {
        // To save tokens.
        // @TODO remove
        return `### **COMPREHENSIVE TACTICAL INTELLIGENCE & OPERATIONS DIRECTIVE**

        **TO:** Planning Agent
        **FROM:** Tactical Intelligence Agent
        **SUBJECT:** Integrated Target Analysis and Automated Assault CONOP
        
        This document merges the raw visual/observational intelligence with the programmatic constraints of our agentic system. Use this data to compile the final execution scripts for the subordinate units.
        
        ---
        
        ### **PART I: TACTICAL INTELLIGENCE ANALYSIS**
        
        #### **1. Forces Overview**
        *   **Enemy Forces (CSAT):** Estimated Platoon-to-Company sized dismounted infantry element (~30-40 personnel). The enemy is localized entirely in the close vicinity of the target compound.
        *   **Enemy Vehicles/Air:** 1x Active Cargo Helicopter spotted on-site. Several unarmored transport trucks are parked within the compound. No heavy enemy armor or mechanized units detected.
        *   **Emplacements:** Multiple static weapon systems identified on the perimeter. Crucially, **Anti-Tank (AT) emplacements are confirmed**.
        *   **Electromagnetic Profile:** Radio recon confirms little-to-no radiation. The target lacks advanced early-warning radar or wide-area jamming capabilities.
        
        #### **2. Positions & Grid Analysis**
        *   **Target Location:** The CSAT Forward Operating Base (FOB) is heavily fortified, centered at approximately **Grid 209-192 to 210-193**. Key identifiers include a prominent white dome structure and hardened perimeter walls.
        *   **Friendly Staging:** Our forces are marshaled in the **Grid 207-174 to 207-175** sector.
        *   **Spatial Relationship:** The target is situated approximately 1.7 to 2 kilometers North-Northeast of our staging area.
        
        #### **3. Terrain Context**
        *   **Topography & Cover:** The operational area is a relatively flat, open agricultural zone. Dense forest cover is absent, though scattered trees, low stone walls, and agricultural hedges exist.
        *   **Lines of Sight:** The open terrain severely advantages the defender. Enemy guard towers and emplaced AT possess unobstructed lines of sight over the primary road networks approaching from the South, West, and East. Direct road approaches are pre-sighted fatal funnels.
        
        #### **4. Overall Assessment**
        *   **Critical Threat:** The static AT emplacements represent an extreme, immediate threat to our mechanized assets (Echo 1-1 APC, Bravo MRAPs). A direct frontal vehicle assault will result in catastrophic losses.
        *   **Strategic Opportunity:** The enemy lacks armor, is clustered inside the walls, and is not running deep patrols. This allows our forces to maneuver into advantageous SBF (Support-by-Fire) and dismount positions without early detection, provided we handle the AT threat first.
        
        ---
        
        ### **PART II: AUTOMATED ASSAULT CONOP**
        
        **Strategic Imperative:** Due to the 2-kilometer distance and open terrain, an entirely dismounted march would leave our assault elements fatigued and exposed. We must utilize Alpha's transport trucks and Echo's IFV to push infantry to a defilade dismount line. Because unit reporting is non-subjective, this plan relies heavily on deterministic \`SyncPoints\` and programmatic \`Event\` triggers.
        
        #### **Global SyncPoints Required**
        *   \`charlie_flank_secure\`
        *   \`alpha1_embark_done\`, \`alpha2_embark_done\`, \`echo_embark_done\`
        *   \`alpha1_dropoff_ready\`, \`alpha2_dropoff_ready\`, \`echo_dropoff_ready\`
        *   \`alpha1_dismounted\`, \`alpha2_dismounted\`, \`echo_dismounted\`
        
        #### **PHASE 1: Shaping & Anti-Armor (The Deterministic Flank)**
        We must dismantle the AT threat and ground the helicopter before main elements enter LOS.
        *   **Execution (Charlie 1-1 & 1-2 AT LSVs):** 
            *   Queue an \`Assault\` task with \`.withCombatBehaviour("COMBAT")\` routing through defilades to flanking overwatch positions (~800m East/West of the FOB). 
            *   Upon reaching the final waypoint, queue \`.signals(charlie_flank_secure)\`.
        *   **Reactive Logic:** Apply a group-level \`.on(Event.ENEMY_CONTACT)\` to Charlie. Engaging \`StaticWeapon\` or \`Helicopter\` is nominal. Apply \`.on(Event.KIA)\`: if \`.getCasualtyRatio() > 0.5\`, they must preempt tasks and \`executeAndClearQueue(new Retreat(...))\`.
        
        #### **PHASE 2: Mechanized Insertion (Strict Transport Contract)**
        Move the dismounts to Grid 208-185 (a depression south of the FOB, out of direct LOS).
        *   **Infantry Elements (Alpha 1-1, Alpha 2-1, Echo 1-2):**
            *   Queue \`Embark(vehicle_reference).signals(embark_done)\` on their respective transports.
            *   Queue a \`Wait(dropoff_ready)\` task.
            *   Queue a \`Disembark().signals(dismounted)\` task.
        *   **Vehicle Elements (Alpha Trucks, Echo 1-1 IFV):**
            *   Queue a \`Wait(embark_done)\` task. *(Critical: No movement prior to this).*
            *   Queue a \`Push\` task with \`.withCombatBehaviour("AWARE")\` to the safe drop-off coordinates, appended with \`.signals(dropoff_ready)\`.
        *   **Reactive Logic:** The Alpha Trucks are unarmored. Apply \`.on(Event.ENEMY_CONTACT)\` to the truck groups while mounted. If enemies are detected en route, they must immediately \`executeAndClearQueue(new Retreat(...))\` to preserve the infantry payload. Add \`.on(Event.TIMEOUT)\` to infantry groups to fallback to a manual \`Push\` if pathing to the vehicle fails.
        
        #### **PHASE 3: Synchronized SBF & Breach**
        This phase executes only when AT is neutralized and infantry is staged.
        *   **Support-by-Fire Elements (Bravo 1-1, Bravo 1-2, Echo 1-1 IFV):**
            *   Queue a \`Wait\` task requiring the \`charlie_flank_secure\` SyncPoint.
            *   Queue an \`Assault\` task to advance into direct LOS of the FOB walls to suppress the perimeter.
        *   **Main Assault Elements (Alpha 1-1, Alpha 2-1, Echo 1-2):**
            *   Queue a \`Wait\` task requiring their respective \`dismounted\` SyncPoints.
            *   Queue an \`Assault\` task with \`.withCombatBehaviour("COMBAT")\` routing directly into the FOB compound to breach and clear.
        *   **Containment Element (Delta 1-1):**
            *   Queue a \`Push\` to the Northern road intersection (Grid 210-195) to block fleeing enemies.
        
        #### **GLOBAL CONTINGENCY (Group-Level Logic)**
        *   **Catastrophic Loss Override:** All groups must possess a persistent \`.on(Event.KIA)\` listener. If any group's \`.getCasualtyRatio()\` reaches \`>= 0.6\` (60% depletion), they must forcefully preempt their active tasks via \`.executeImmediately(new Retreat(...))\` to fall back to the Grid 207-174 staging area.`;
