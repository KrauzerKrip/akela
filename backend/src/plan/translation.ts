...
export function translateToPlanGroup(plan: Plan, group: Group): PlanGroup {
  ...
  return {
    ...
    enqueue(jsTask) {
      ...
      console.log(
        `[Translation] enqueue: jsTask.id=${jsTask.id}, translatedTaskId=${task.id}`,
      );
      ...
      plan.taskReactions[task.id] = {
        ...groupReactions,
        ...(jsTask.reactions || {}),
      };

      console.log(
        `[Translation] enqueue: Registered reactions for task ${task.id}:`,
        Object.keys(plan.taskReactions[task.id]),
      );
    ...
    executeImmediately(jsTask) {
      const task = translateTask(jsTask);
      console.log(
        `[Translation] executeImmediately: jsTask.id=${jsTask.id}, translatedTaskId=${task.id}`,
      );
      ...
      console.log(
        `[Translation] executeImmediately: Registered reactions for task ${task.id}:`,
        Object.keys(plan.taskReactions[task.id]),
      );
    ...
    executeAndClearQueue(jsTask) {
      const task = translateTask(jsTask);
      console.log(
        `[Translation] executeAndClearQueue: jsTask.id=${jsTask.id}, translatedTaskId=${task.id}`,
      );
      ...
      console.log(
        `[Translation] executeAndClearQueue: Registered reactions for task ${task.id}:`,
        Object.keys(plan.taskReactions[task.id]),
      );
...
export function translateTask(jsTask: any): Task {
  ...
  let waypoints = [];
  if (jsTask.waypoints) {
    waypoints = jsTask.waypoints.map(
      (wp: any) =>
        ({
          id: uuidv4(),
